...
class LensTests: XCTestCase {
	...
	func testOver() {
		property("'over' works like injecting a value dependent on the previous tryGet") <- forAll { (p: Pair<Int,Int>, ar: ArrowOf<Int,Int>) in
			let a = ar.getArrow
			return Pair<Int,Int>.lens.a.over(a)(p)
